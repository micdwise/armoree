...
const GetAllCalibers = async () => {
    const { data, error } = await supabase.from("reference_calibers").select("*").order("name");
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (editingItem) {
                ...
                const newModel = await AddModel(Number.parseInt(selectedManufacturerId), newItemName);
        ...
        <div className="space-y-4">
            <div className="flex flex-col sm:flex-row gap-4 justify-between items-end sm:items-center">
                <div className="w-full sm:w-64">
                    ...
                    <Select
                        ...
                        options={manufacturers.map(m => ({ value: m.manufacturer_id.toString(), label: m.name }))}
            ...
            <div className="rounded-md border border-neutral-border bg-default-background">
                <Table>
                    ...
                    <TableBody>
                        {isLoading ? (
                            <TableRow>
                                <TableCell colSpan={3} className="text-center py-8"><Spinner /></TableCell>
                            </TableRow>
                        ) : models.length === 0 ? (
                            <TableRow>
                                <TableCell colSpan={3} className="text-center py-8 text-subtext-color">No models found for this manufacturer.</TableCell>
                            </TableRow>
                        ) : (
                            models.map((m) => (
                                <TableRow key={m.model_id}>
                                    <TableCell>{m.name}</TableCell>
                                    <TableCell className="text-subtext-color text-sm">Use edit to view</TableCell>
            ...
            >
                <div className="p-4 space-y-6">
                    ...
                    <div>
                        <label className="text-sm font-medium mb-2 block">Valid Calibers</label>
                        <div className="border border-neutral-border rounded p-3 h-48 overflow-y-auto grid grid-cols-2 gap-2">
                            {allCalibers.map(c => (
                                <label key={c.caliber_id} className="flex items-center space-x-2 text-sm cursor-pointer hover:bg-screen-background p-1 rounded">
                                    <input
                                        type="checkbox"
                        ...
                        <p className="text-xs text-subtext-color mt-1">Select all calibers compatible with this model.</p>
                    </div>
                </div>
            </Modal>
        </div>
