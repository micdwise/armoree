...
interface ExpiringQualificationsCardProps {
    count: number;
    isLoading?: boolean;
}

export const ExpiringQualificationsCard: React.FC<ExpiringQualificationsCardProps> = ({ count, isLoading }) => {
    if (isLoading) {
        return (
        ...
        <Link to="/Personnel?filter=expiring">
            <Card className="hover:bg-neutral-50 dark:hover:bg-neutral-900 transition-colors cursor-pointer h-full">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    ...
                    <BadgeAlert className={clsx("h-4 w-4", severity)} />
                </CardHeader>
                <CardContent>
