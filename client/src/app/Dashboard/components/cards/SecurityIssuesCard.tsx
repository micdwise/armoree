...
interface SecurityIssuesCardProps {
    count: number;
    isLoading?: boolean;
}

export const SecurityIssuesCard: React.FC<SecurityIssuesCardProps> = ({ count, isLoading }) => {
    if (isLoading) {
        return (
        ...
        <Link to="/Settings/Security?filter=issues">
            <Card className="hover:bg-neutral-50 dark:hover:bg-neutral-900 transition-colors cursor-pointer h-full">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    ...
                    <AlertTriangle className={clsx("h-4 w-4", severity)} />
                </CardHeader>
                <CardContent>
