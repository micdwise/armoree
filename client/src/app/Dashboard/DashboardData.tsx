/**
 * Dashboard queries - All data is tenant-scoped.
...
async function getLowAmmoStockCount(): Promise<number> {
  ...
  return data.filter(
    (item) => item.quantity_on_hand < (item.min_stock_level || 0)
  ).length;
}
...
async function getInventoryStatusDistribution(): Promise<
...
async function getHighWearWeaponReport(): Promise<any[]> {
  ...
  usageData?.forEach((log) => {
    roundsByFirearm[log.firearm_id] =
      (roundsByFirearm[log.firearm_id] || 0) + log.rounds_fired;
  });
...
async function getAmmunitionExpenditureByUnit(): Promise<
  { unit_name: string; total_rounds: number }[]
> {
  // Join usage_log -> personnel -> unit
  // Fetch flattened data
  const { data, error } = await supabase.from("usage_log").select(`
            rounds_fired,
            personnel (
                unit (
                    unit_name
  ...
  return Object.entries(roundsByUnit).map(([unit_name, total_rounds]) => ({
    unit_name,
    total_rounds,
  }));
}
