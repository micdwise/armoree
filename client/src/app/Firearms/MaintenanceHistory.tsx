...
import { Table, TableHeader, TableBody, TableRow, TableHead, TableCell, Card, CardContent, Spinner, Button } from "@components";
...
export const MaintenanceHistory: React.FunctionComponent<MaintenanceHistoryProps> = ({ firearmId }) => {
  const params = useParams<{ id: string }>();
  const id = firearmId ? String(firearmId) : params.id;
  const { data: maintenanceLogs, isLoading, isError, refetch } = useMaintenanceLogs(id);
    ...
    <Card>
      <CardContent className="p-6">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-semibold text-default-font">Maintenance History</h3>
        ...
        {isLoading ? (
          ...
          <p className="text-sm text-subtext-color">No history available yet.</p>
        ) : (
              ...
              <TableBody>
                {maintenanceLogs.map((log) => (
                  <TableRow key={log.log_id}>
                    ...
                    <TableCell>
                      {log.personnel ? `${log.personnel.first_name} ${log.personnel.last_name}` : "Unknown"}
                    </TableCell>
